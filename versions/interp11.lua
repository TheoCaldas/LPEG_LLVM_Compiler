...
local suffixed = lpeg.V"suffixed"
...
local syntax = lpeg.P{"defs";
  ...
  primary = 
    ...
    id / node("uVAR", "id") +
    (OP * lpeg.V"exp" * CP);
  suffixed = call + primary;
  factor = suffixed + ((opN * suffixed) / node("UAO", "op", "e"));
...
function Compiler:codeExp (exp)
  ...
  elseif tag == "call" then
    if not self.funcs[exp.name] then
      error("unknown function " .. exp.name)
    end
