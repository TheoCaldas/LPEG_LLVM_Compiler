...
local syntax = lpeg.P{"defs";
  defs = lpeg.Ct(def^1);
  def = rw"fun" * id * OP * CP * opt(C * id) * block / node("func", "name", "optType", "body");
...
function Compiler:codeExp (exp)
  ...
  elseif tag == "call" then
    if not self.functions[exp.name] then
      errorMsg("unknown function " .. exp.name)
    end
...
function Compiler:codeStat (st)
  ...
  elseif tag == "call" then
    if not self.functions[st.name] then
      errorMsg("unknown function " .. st.name)
    end
    local reg = self:newTemp()
    formatWrite("  %s = call i32 @%s()\n", reg, st.name)
    return reg
