...
local syntax = lpeg.P{"defs";
  defs = lpeg.Ct(def^1);
  def = rw"fun" * id * OP * opt(id) * CP * opt(C * id) * block / node("func", "name", "optArg", "optType", "body");
  ...
  stat = 
    ...
    call;
  call = id * OP * opt(exp) * CP / node("call", "name", "optParam");
  primary = 
...
function Compiler:codeParam (param)
  if isEmpty(param) then
    return ")\n"
  end
  local r = self:codeExp(param)
  return string.format("i32 %s)\n", r)
end
...
function Compiler:codeCall(call)
  ...
  local param = self:codeParam(call.optParam)
  local reg = self:newTemp()
  formatWrite("  %s = call i32 @%s(%s", reg, call.name, param)
  return reg
end
...
function Compiler:codeExp (exp)
  ...
  elseif tag == "call" then
    if self.functions[exp.name] == "void" then
...
function Compiler:codeStat (st)
  ...
  elseif tag == "return" then
    currentType = self.functions[self.currentFunc]
