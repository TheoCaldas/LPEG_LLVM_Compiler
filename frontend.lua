...
-- MARK: Local Vars
local reservedWords = {"var", "ret", "fun", "if", "else", "while", "as", "for", "in"}
local lastpos = 0
...
local syntax = lpeg.P{"defs";
  ...
  stat = 
    ...
    ( (rw"for" * id * rw"in" * opt((integer / node("INT", "num")) * CM) * (integer / node("INT", "num")) * opt(CM * (integer / node("INT", "num"))) * block) / node("for", "counter", "optStart", "stop", "optStep", "block")) + 
    call +
    comment;
  rawType = id;
  typed = CL * rawType;
