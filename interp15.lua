...
local Compiler = {
  ...
  BAOmap = function(op, tp){
    if op == "+" and tp == Compiler.dataTypes.int then
...
function Compiler:codeExp (exp) 
  -- return result expression followed by its data type
  local tag = exp.tag
  if tag == "INT" then
    return string.format("%d", exp.num), Compiler.dataTypes.int
  elseif tag == "FLOAT" then
    return string.format("%f", exp.num), Compiler.dataTypes.float
  elseif tag == "uVAR" then
    ...
    return temp
  elseif tag == "UAO" then
    local eResult, eType = self:codeExp(exp.e)
    local temp = self:newTemp()
    if eType == Compiler.dataTypes.int then
      formatWrite("  %s = sub i32 0, %s\n", temp, eResult)
    elseif eType == Compiler.dataTypes.float then
      formatWrite("  %s = fneg double %s\n", temp, eResult)
    else
      errorMsg(tag .. " not defined for " .. eType)
    end
    return temp
  elseif tag == "BAO" then
    local eResult_1, eType_1 = self:codeExp(exp.e1)
    local eResult_2, eType_2 = self:codeExp(exp.e2)
    local temp = self:newTemp()
    if eType_1 ~= eType_2 then
      errorMsg(eType_1 .. " " .. exp.op .. " " .. eType_2 .. " is not defined")
    elseif eType_1 == Compiler.dataTypes.int then
      formatWrite("  %s = %s i32 %s, %s\n", temp, self.BAOmap[exp.op], eResult_1, eResult_2)
    elseif eType_1 == Compiler.dataTypes.float then

    else
      errorMsg(exp.op .. " not defined for " .. eType_1)
    end
    return temp
  elseif tag == "BCO" then
